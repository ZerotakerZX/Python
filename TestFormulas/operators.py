...
def task12198(ar0):
    a = np.sum(ar0,axis=0) #есть особая команда, что позволяте суммировать каждое значение по какой-то оси
    return list(a)

def task12200(ar0, k, s):
    ar0[k] = np.negative(ar0[1]) #берём заданную строку и обращаем весь ряд, включая цирфу на пересечении
    ar0[:,s] = np.negative(ar0[:,1]) #берём заданную колонку и обращаем весь ряд, включая цирфу на пересечении
    return ar0.tolist() #цифра на пересечении обращается дважды, т.е. не меняется и возвращаем для удобсвта в виде списка что вышло

def task12201(ar0):
    for i in range(len(ar0[0])): #делаем столько проходов, сколько строк в массиве
        a = ar0[i, 0] #и каждый будем разбирать отдельно, сперва находим первое число в строке
        b = np.amax(ar0[i]) #и значение максимального
        bi = np.where(ar0 == np.amax(ar0[i])) #а так же его индекс
        bi = (bi[1]) #форматируем индекс в нежный формат
        ar0[i, 0] = b #первое число в строке заменяем ранее сохранёнм максимальным
        ar0[i, bi] = a #и по индексу максимального числа записываем то, что было первым в строке
    return ar0.tolist() #форматируем ответ в лист перед отправкой

def task12205a(ar0, a, b):
    tmp0 = ar0[a] #временное хралище одной строки
    tmp1 = ar0[b] #и другой
    ar0 = np.row_stack((tmp1, tmp0)) #проще создать новый список поверх старого
    return ar0.tolist()  # форматируем ответ в лист перед отправкой

def task12206a(ar0): #говно не решается
    print(ar0)
    print(ar0[1, ::2])
    for i in (0, 1):
        ar0[i, ::1] = (ar0[i, 0] + ar0[i, ::2])
    print(ar0)
    return [[0, 0], [4, 4]]