...
class TestOperators(unittest.TestCase):
    ...
    def test_task121(self): # Вывести на экран элемент, расположенный в правом верхнем углу массива
        expected_result = 3
        ar0 = [[1, 2, 3], [4, 5, 6]] #массив в виде вложенного списка
        result = task121(ar0)
        assert_equal(expected_result, result)

    def test_task121b(self): # Вывести на экран элемент, расположенный в левом нижнем углу массива
        expected_result = 4
        ar0 = [[1, 2, 3], [4, 5, 6]] #массив в виде вложенного списка
        result = task121b(ar0)
        assert_equal(expected_result, result)

    def test_task123a(self): # составить прогу вывода на экран любого элемента второй строки массива;
        expected_result = 4
        ar0 = [[1, 2, 3], [4, 5, 6]] #массив в виде вложенного списка
        inp = int(input("Введите номер символа")) #для теста нужно ввести 1
        result = task123a(ar0, inp)
        assert_equal(expected_result, result)

    def test_task125b(self): # составить прогу что выводит все элементы s-го столбца массива
        expected_result = [1, 4]
        ar0 = np.array([[1, 2, 3], [4, 5, 6]]) #массив в виде для модуля numpy
        inp = int(input("Введите номер символа"))  # для теста нужно ввести 1
        result = task125b(ar0, inp)
        assert_equal(expected_result, result)

    def test_task127a(self): #написать прогу заменяющую значение любого элемента второй строки двумерного массива числом 1949
        expected_result = 150
        ar0 = np.array([[1, 2, 3], [4, 5, 6]]) #массив в виде для модуля numpy
        inp = int(input("Введите номер символа"))  # для теста нужно ввести 1
        result = task127a(ar0, inp)
        assert_equal(expected_result, result)

    def test_task128a(self): #написать прогу заменяющую значение любого элемента второго столбца двумерного массива числом 13;
        expected_result = 13
        ar0 = np.array([[1, 2, 3], [4, 5, 6]]) #массив в виде для модуля numpy
        inp = int(input("Введите номер символа"))  #для теста нужно ввести 1
        result = task128a(ar0, inp)
        assert_equal(expected_result, result)

    '''def test_task129a(self): #Дан двумерный массив. Поменять местами: элементы, расположенные в правом верхнем и левом нижнем углах;
        expected_result = (4, 3)
        ar0 = np.array([[1, 2, 3], [4, 5, 6]]) #массив в виде для модуля numpy
        result = task129a(ar0)
        assert_equal(expected_result, result)'''

    def test_task1222(self): #Заполнить двумерный массив результатами таблицы умножения (в первойстроке должны быть записаны роизведения каждого из чисел от 1 до 9 на 1,во второй — на 2, ..., в последней — на 9).
    ...
    def test_task1226a(self): #Заполнить двумерный массив размером n n единицами и нулями таким образом, чтобы единицы размещались так, как размещаются на шахматной доске черные поля, а нули — как белые поля. Левое нижнее поле на шахматной доске всегда черное. Задачу решить: а) при четном значении n;
        expected_result = 0.0 #последнее число для сравнения
        n = 8
        ar0 = np.empty([n, n])  # пустой массив в виде для модуля numpy
        result = task1226a(ar0, n)
        assert_equal(expected_result, result)

    def test_task1227(self): #Дан двумерный массив из m строк и n столбцов. Заполнить его значениями элементов одномерного массива размером m n . Заполнение проводить по строкам, начиная с первой (а в ней — начиная с первого элемента).
        expected_result = 1.0 #последнее число для сравнения
        n = 2
        m = 4
        ar0 = np.empty([n, m])  # пустой массив в виде для модуля numpy
        ls0 = np.empty([1, (n * m)])
        result = task1227(ar0, ls0, n, m)
        assert_equal(expected_result, result)

    def test_task1232a(self): #Дан двумерный массив. Вывести на экран:а) все элементы третьей строки массива, начиная с последнего элемента этойстроки
        expected_result = [10,  9,  8]
        ar0 = np.array([[1,2,3],[5,6,7],[8,9,10]])  # простенький массивчик для работы
        result = task1232a(ar0)
        assert_equal(expected_result, result)

    def test_task1232b(self): #все элементы k-го столбца массива, начиная с нижнего элемента этого столблца
        expected_result = [10, 7, 3]
        k = 3
        ar0 = np.array([[1,2,3],[5,6,7],[8,9,10]])  # простенький массивчик для работы
    ...
    def test_task12110(self): #теплыми являются дни с 1 по 7 августа. выбрать 3 следующих подряд дней этого периода, которые были наиболее теплыми за последние 3 лет
        expected_result = (5, 6, 7)
        ar0 = np.array([[1,2,3,4,5,6,7],[11,12,13,14,15,16,17],[21,22,23,24,25,26,27]])  # простенький массивчик для работы
        result = task12110(ar0)
        assert_equal(expected_result, result)

    def test_task12134(self): #В двумерном массиве имеются только два одинаковых элемента. Найти их
        expected_result = [(1, 0), (2, 0)]
        ar0 = np.array([[1,2,3,4,5,6,7],[99,12,13,14,15,16,17],[99,22,23,24,25,26,27]])  # простенький массивчик для работы
        result = task12134(ar0)
        assert_equal(expected_result, result)

    def test_task12154(self): #Известен номер строки, на которой расположен элемент главной диагонали квадратного массива. Вывести на экран значение этого элемента
        expected_result = 8
        string_number = 3 #известный номер строки
        ar0 = np.arange(9).reshape((3,3))  #сгенерировать КВАДРАТНЫЙ массивчик для работы
        result = task12154(ar0, string_number)
        assert_equal(expected_result, result)

    def test_task12155(self): #Известен номер строки, на которой расположен элемент главной диагонали квадратного массива. Вывести на экран значение этого элемента
        expected_result = 6
        string_number = 3 #известный номер строки
        ar0 = np.arange(9).reshape((3,3))  #сгенерировать КВАДРАТНЫЙ массивчик для работы
        result = task12155(ar0, string_number)
        assert_equal(expected_result, result)

    def test_task12160(self): #Заменить значения всех элементов главной диагонали квадратного массиванулевыми.
        expected_result = 0
        ar0 = np.arange(9).reshape((3,3))  #сгенерировать КВАДРАТНЫЙ массивчик для работы
        result = task12160(ar0)
        assert_equal(expected_result, result)

    def test_task12177(self): #В квадратном массиве из n строк, где n — нечетное число, все элементы различны. Наибольший элемент среди стоящих на главной и побочной диагоналях поменять местами с элементом, стоящим на пересечении этих диагоналей.
        expected_result = 4
        ar0 = np.arange(9).reshape((3,3))  #сгенерировать КВАДРАТНЫЙ массивчик для работы
